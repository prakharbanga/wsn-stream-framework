...
\\
The frameworks given described till assume that there is no hierarchy in the network. Firecracker[12] works for a hierarchichal network and is a multi-hop framework. All of these frameworks have been designed for Berkeley Motes and work on TinyOS environment. Sprinkler[13] is a framework using TDMA as Mac protocol for multi-hop networks and is not designed for the Motes in particular.
\\
All these frameworks can be put in different categories as described above in the various sections. We will be aiming towards a general framework with least assumptions for the network.
